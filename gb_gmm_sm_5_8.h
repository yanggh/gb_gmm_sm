...
{    
    ...
    if(len > 0)
    {
        ...
        B8_BIT(tmp, 8, 8);
        if(tmp == 0)
            A_B8_BIT2(gbf, gea1, "GEA1 bits", 8, 8, "encryption algorithm GEA/1 not available");
        else
            A_B8_BIT2(gbf, gea1, "GEA1 bits", 8, 8, "encryption algorithm GEA/1 available");
        B8_BIT(tmp, 7, 7);
        if(tmp == 0)
        {
...
{    
    ...
    for(i = 0; i  < 6; i--)
    {
        ...
        if(tmp == 0)
        {
            A_B8_BIT3(gbf, capability_o, apc_otd_gps[i], i, i, ps_lcs_capability_o);
        }
        else
        {
            A_B8_BIT3(gbf, capability_i, apc_otd_gps[i], i, i, ps_lcs_capability_i);
        }
    }    
    SKIP(1);
...
{ 
    ...
    {
        B8_HEX();
        A_B8_BIT1(gbf, nri_container_value, "NRI container value", 1, 8);
        len -= 1;
        SKIP(1);
    }
...
void network_feature_support(uint8_t iei, uint8_t location)
{
    ...
    if(iei == 0)
    {
        ...
        else
        {
            ...
            if(tmp == 1)
            {
                A_B8_BIT2(gbf, emcbs, "EMCBS", 5, 5,  "Emergency bearer services in Iu mode and A/Gb mode supported");
            }
            else
            {
...
void pdp_context_status(uint8_t iei, uint8_t location)
{
    ...
    if(len > 0)
    {
        ...
        {
            bzero(buf, 16);
            sprintf(buf, "NSAP(%d)", i-1);
            B8_BIT(tmp, i, i);
            if(tmp == 1)
            {
                A_B8_BIT2(gbf, nsapi, buf, i, i, "indicates that the SM state of the corresponding PDP context is not PDP-INACTIVE.");
            }
            else
            {
                A_B8_BIT2(gbf, nsapi, buf, i, i, "indicates that the SM state of the corresponding PDP context is PDP-INACTIVE");     
            }
        }
...
void uplink_data_status(uint8_t iei, uint8_t location)
{
    ...
    {
        bzero(buf, 16);
        sprintf(buf, "NSAPI(%d)", i-1 + j * 8);
        B8_BIT(tmp, i, i);
        if(tmp == 0)
        {
            A_B8_BIT2(gbf, nsapi, buf, i, i, "no uplink data are pending for the preserved PDP context or the PDP context is PDP-INACTIVE or is PDP-ACTIVE with a RAB already established.");
        }
        else
        {
            A_B8_BIT2(gbf, nsapi, buf, i, i, "1 uplink data are pending for the preserved PDP context.");   
        }
...
void quality_of_service(uint8_t iei, uint8_t location, int ms_to_net)
{
    ...
    if(len > 0)
    {
        ...
        if(tmp == 0)
        {
            if(ms_to_net == 1)
            {
                A_B8_BIT2(gbf, mean_throughput, "Mean throughput", 1, 5, "");
            }
            else
            {
                A_B8_BIT2(gbf, mean_throughput, "Mean throughput", 1, 5, "");
    ...
    if(len > 0)
    {
        ...
        if(tmp == 0)
        {
            if(ms_to_net == 1)
            {
                A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "Subscribed maximum bit rate for downlink");
            }
            else
            {
                A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "Reserved");
            }
        }
        else if(tmp == 1)
        {
            A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "The maximum bit rate is binary coded in 8 bits, using a granularity of 1 kbps");
        }
        else if(tmp == 63)
        {
            A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "giving a range of values from 1 kbps to 63 kbps in 1 kbps increments.");
        }
        else if(tmp == 64)
        {
            A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "The maximum bit rate is 64 kbps + ((the binary coded value in 8 bits –01000000) * 8 kbps)");
        }
        else if(tmp == 172)
        {
            A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "giving a range of values from 64 kbps to 568 kbps in 8 kbps increments.");
        }
        else if(tmp == 128)
        {
            A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "The maximum bit rate is 576 kbps + ((the binary coded value in 8 bits –10000000) * 64 kbps)");
        }
        else if(tmp == 376)
        {
            A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "giving a range of values from 576 kbps to 8640 kbps in 64 kbps increments.");
        }
        else if(tmp == 255)
        {
            A_B8_BIT2(gbf, maximum_bit_rate_for_uplink, "Maximum bit rate for downlink", 1, 8, "0kbps");
        }
        len -= 1;
        SKIP(1);
    }
    ...
    if(len > 0)
    {
        ...
        {
            A_B8_BIT2(gbf, residual_bit_error_rate, "Residual Bit Error Rate", 5, 8, "7*10^-3");   
        }
        else if(tmp  == 3)
        {
            A_B8_BIT2(gbf, residual_bit_error_rate, "Residual Bit Error Rate", 5, 8, "1*10^-3");   
        ...
        {
            A_B8_BIT2(gbf, residual_bit_error_rate, "Residual Bit Error Rate", 5, 8, "1*10^-1");   
        }
        else
        {
            A_B8_BIT2(gbf, residual_bit_error_rate, "Residual Bit Error Rate", 5, 8, "Reserved");   
    ...
    if(len > 0)
    {
        ...
        {
            A_B8_BIT2(gbf, transfer_delay, "Transfer delay", 3, 8, "The transfer delay is 200 ms + ((the binary coded value in 6 bits – 010000) * 50 ms)");        
        }
        else if(tmp == 31)
        {
    ...
    {
        B8_HEX();
        A_B8_BIT1(gbf, maximum_bit_rate_for_uplink_extended, "Maximum bit rate for uplink (extended)", 1, 8);
        len -= 1;
        SKIP(1);
    }
    ...
    {
        B8_HEX();
        A_B8_BIT1(gbf, guaranteed_bit_rate_for_uplink_extended, "Guaranteed bit rate for uplink (extended)", 1, 8);
        len -= 1;
        SKIP(1);
    }
