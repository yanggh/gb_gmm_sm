...
#include "nbe_pu_gbf.h"
#include "nbp_resume_nack.h"

#define NPCTX      struct resume_nack_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "RESUME-NACK");
...
extern "C" {
#endif
void *resume_nack_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct resume_nack_ctx));
    if (!pState) {
...
void resume_nack_state_free(void *state) {
...
nbe_pu_gb *resume_nack_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int resume_nack_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int resume_nack_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int resume_nack_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
