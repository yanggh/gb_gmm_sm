...
#include "nbe_pu_gbf.h"
#include "nbp_notification.h"

#define NPCTX      struct notification_ctx*
...
{{ 
	...
	BEGIN_SIGNAL("4", "SM", "NOTIFICATION");
    ...
    notification_indicator(iei, location);
...
void *notification_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct notification_ctx));
    if (!pState) {
