...
#include "nbe_pu_gbf.h"
#include "nbp_routing_area_update_complete.h"

#define NPCTX      struct routing_area_update_complete_ctx*
...
struct routing_area_update_complete_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "GMM", "ROUTING AREA UPDATE COMPLETE");
...
extern "C" {
#endif
void *routing_area_update_complete_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct routing_area_update_complete_ctx));
    if (!pState) {
...
void routing_area_update_complete_state_free(void *state) {
...
nbe_pu_gb *routing_area_update_complete_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int routing_area_update_complete_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int routing_area_update_complete_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int routing_area_update_complete_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
