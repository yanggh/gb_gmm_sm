...
#include "nbe_pu_gbf.h"
#include "nbp_detach_accept.h"

#define NPCTX      struct detach_accept_ctx*
...
extern "C" {
#endif
void *detach_accept_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct detach_accept_ctx));
    if (!pState) {
...
void detach_accept_state_free(void *state) {
...
nbe_pu_gb *detach_accept_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int detach_accept_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int detach_accept_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int detach_accept_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
