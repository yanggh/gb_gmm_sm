...
nbe_pu_sndcp *nbe_sndcp_new(void) {
    uint32_t extra = 4096;
    uint32_t fixed_size = sizeof(nbe_pu_sndcp);
    nbe_pu_sndcp *pu = (nbe_pu_sndcp*) malloc(fixed_size +extra);
    check_pu_new(pu);
...
void nbe_sndcp_set_gb_layer_post(void *v, nbe_ptr p, int delta) {
   nbe_pu_sndcp *pu = (nbe_pu_sndcp*) v;
...
nbe_def *nbe_get_pu_def_195(uint32_t uid);
nbe_def *nbe_get_pu_def_195(uint32_t uid) {
    nbe_pu_sndcp pu;
