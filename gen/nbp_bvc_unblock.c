...
#include "nbe_pu_gbf.h"
#include "nbp_bvc_unblock.h"

#define NPCTX      struct bvc_unblock_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "BVC-UNBLOCK");
...
extern "C" {
#endif
void *bvc_unblock_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct bvc_unblock_ctx));
    if (!pState) {
...
void bvc_unblock_state_free(void *state) {
...
nbe_pu_gb *bvc_unblock_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int bvc_unblock_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int bvc_unblock_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int bvc_unblock_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
