...
#include "nbe_pu_gbf.h"
#include "nbp_resume.h"

#define NPCTX      struct resume_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "RESUME");

      int len = 18;
      bssgp_pdu_type();
	  bssgp_tlli(1, 1);
      routeing_area(1, 1);
      suspend_reference_number(1, 1);
...
void *resume_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct resume_ctx));
    if (!pState) {
