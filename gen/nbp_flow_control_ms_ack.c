...
#include "nbe_pu_gbf.h"
#include "nbp_flow_control_ms_ack.h"

#define NPCTX      struct flow_control_ms_ack_ctx*
...
struct flow_control_ms_ack_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "FLOW-CONTROL-MS-ACK");
...
void *flow_control_ms_ack_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct flow_control_ms_ack_ctx));
    if (!pState) {
