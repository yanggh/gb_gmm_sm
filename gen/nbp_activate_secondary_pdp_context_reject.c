...
#include "nbe_pu_gbf.h"
#include "nbp_activate_secondary_pdp_context_reject.h"

#define NPCTX      struct activate_secondary_pdp_context_reject_ctx*
...
struct activate_secondary_pdp_context_reject_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "SM", "ACTIVATE SECONDARY PDP CONTEXT REJECT");
...
void *activate_secondary_pdp_context_reject_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct activate_secondary_pdp_context_reject_ctx));
    if (!pState) {
