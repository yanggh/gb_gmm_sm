...
#include "nbe_pu_gbf.h"
#include "nbp_service_reject.h"

#define NPCTX      struct service_reject_ctx*
...
{{ 
	...
	BEGIN_SIGNAL("4", "GMM", "SERVICE REJECT");
...
void *service_reject_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct service_reject_ctx));
    if (!pState) {
