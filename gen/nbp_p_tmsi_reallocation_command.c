...
#include "nbe_pu_gbf.h"
#include "nbp_p_tmsi_reallocation_command.h"

#define NPCTX      struct p_tmsi_reallocation_command_ctx*
...
struct p_tmsi_reallocation_command_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "GMM", "P-TMSI REALLOCATION COMMAND");
...
extern "C" {
#endif
void *p_tmsi_reallocation_command_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct p_tmsi_reallocation_command_ctx));
    if (!pState) {
...
void p_tmsi_reallocation_command_state_free(void *state) {
...
nbe_pu_gb *p_tmsi_reallocation_command_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int p_tmsi_reallocation_command_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int p_tmsi_reallocation_command_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int p_tmsi_reallocation_command_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
