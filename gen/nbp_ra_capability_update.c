...
#include "nbe_pu_gbf.h"
#include "nbp_ra_capability_update.h"

#define NPCTX      struct ra_capability_update_ctx*
...
struct ra_capability_update_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "RA-CAPABILITY-UPDATE");
...
void *ra_capability_update_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct ra_capability_update_ctx));
    if (!pState) {
