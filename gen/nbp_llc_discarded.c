...
#include "nbe_pu_gbf.h"
#include "nbp_llc_discarded.h"

#define NPCTX      struct llc_discarded_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "LLC-DISCARDED");
...
void *llc_discarded_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct llc_discarded_ctx));
    if (!pState) {
