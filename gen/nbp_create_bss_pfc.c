...
#include "nbe_pu_gbf.h"
#include "nbp_create_bss_pfc.h"

#define NPCTX      struct create_bss_pfc_ctx*
...
extern "C" {
#endif
void *create_bss_pfc_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct create_bss_pfc_ctx));
    if (!pState) {
...
void create_bss_pfc_state_free(void *state) {
...
nbe_pu_gb *create_bss_pfc_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int create_bss_pfc_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int create_bss_pfc_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int create_bss_pfc_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
