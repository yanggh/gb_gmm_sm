...
#include "nbe_pu_gbf.h"
#include "nbp_identity_response.h"

#define NPCTX      struct identity_response_ctx*
...
{{ 
	...
	BEGIN_SIGNAL("4", "GMM", "IDENTITY RESPONSE");
...
extern "C" {
#endif
void *identity_response_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct identity_response_ctx));
    if (!pState) {
...
void identity_response_state_free(void *state) {
...
nbe_pu_gb *identity_response_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int identity_response_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        default:
            return -1;
...
int identity_response_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        default:
            return -1;
...
int identity_response_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
