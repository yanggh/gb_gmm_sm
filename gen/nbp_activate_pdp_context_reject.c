...
#include "nbe_pu_gbf.h"
#include "nbp_activate_pdp_context_reject.h"

#define NPCTX      struct activate_pdp_context_reject_ctx*
...
struct activate_pdp_context_reject_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "SM", "ACTIVATE PDP CONTEXT REJECT");
...
extern "C" {
#endif
void *activate_pdp_context_reject_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct activate_pdp_context_reject_ctx));
    if (!pState) {
...
void activate_pdp_context_reject_state_free(void *state) {
...
nbe_pu_gb *activate_pdp_context_reject_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int activate_pdp_context_reject_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int activate_pdp_context_reject_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int activate_pdp_context_reject_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
