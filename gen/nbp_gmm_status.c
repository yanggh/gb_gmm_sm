...
#include "nbe_pu_gbf.h"
#include "nbp_gmm_status.h"

#define NPCTX      struct gmm_status_ctx*
...
extern "C" {
#endif
void *gmm_status_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct gmm_status_ctx));
    if (!pState) {
...
void gmm_status_state_free(void *state) {
...
nbe_pu_gb *gmm_status_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int gmm_status_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int gmm_status_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int gmm_status_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
