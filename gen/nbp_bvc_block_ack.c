...
#include "nbe_pu_gbf.h"
#include "nbp_bvc_block_ack.h"

#define NPCTX      struct bvc_block_ack_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "BVC-BLOCK-ACK");
...
extern "C" {
#endif
void *bvc_block_ack_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct bvc_block_ack_ctx));
    if (!pState) {
...
void bvc_block_ack_state_free(void *state) {
...
nbe_pu_gb *bvc_block_ack_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int bvc_block_ack_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int bvc_block_ack_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int bvc_block_ack_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
