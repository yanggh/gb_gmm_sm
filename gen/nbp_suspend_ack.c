...
#include "nbe_pu_gbf.h"
#include "nbp_suspend_ack.h"

#define NPCTX      struct suspend_ack_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "SUSPEND-ACK");
...
void *suspend_ack_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct suspend_ack_ctx));
    if (!pState) {
