...
#include "nbe_pu_gbf.h"
#include "nbp_modify_bss_pfc.h"

#define NPCTX      struct modify_bss_pfc_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "MODIFY-BSS-PFC");
...
void *modify_bss_pfc_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct modify_bss_pfc_ctx));
    if (!pState) {
