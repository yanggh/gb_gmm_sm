...
#include "nbe_pu_gbf.h"
#include "nbp_status.h"

#define NPCTX      struct status_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "STATUS");

      uint8_t h = 0;
      int len = 4;
      int pdu_in_error_len = 0;
      bssgp_pdu_type();
      bssgp_cause(1, 1);
	  ...
	  if (h == 0x15)
	  {
		pdu_in_error_len = pdu_in_error(1, 1);
        len += pdu_in_error_len;
      }
...
void *status_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct status_ctx));
    if (!pState) {
