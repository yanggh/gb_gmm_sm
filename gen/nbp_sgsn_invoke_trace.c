...
#include "nbe_pu_gbf.h"
#include "nbp_sgsn_invoke_trace.h"

#define NPCTX      struct sgsn_invoke_trace_ctx*
...
extern "C" {
#endif
void *sgsn_invoke_trace_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct sgsn_invoke_trace_ctx));
    if (!pState) {
...
void sgsn_invoke_trace_state_free(void *state) {
...
nbe_pu_gb *sgsn_invoke_trace_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int sgsn_invoke_trace_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int sgsn_invoke_trace_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int sgsn_invoke_trace_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
