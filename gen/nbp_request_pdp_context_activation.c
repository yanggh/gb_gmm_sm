...
#include "nbe_pu_gbf.h"
#include "nbp_request_pdp_context_activation.h"

#define NPCTX      struct request_pdp_context_activation_ctx*
...
struct request_pdp_context_activation_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "SM", "REQUEST PDP CONTEXT ACTIVATION");
...
void *request_pdp_context_activation_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct request_pdp_context_activation_ctx));
    if (!pState) {
