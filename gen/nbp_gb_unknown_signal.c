...
#include "nbe_pu_gbf.h"
#include "nbp_gb_unknown_signal.h"

#define NPCTX      struct gb_unknown_signal_ctx*
...
extern "C" {
#endif
void *gb_unknown_signal_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct gb_unknown_signal_ctx));
    if (!pState) {
...
void gb_unknown_signal_state_free(void *state) {
...
nbe_pu_gb *gb_unknown_signal_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int gb_unknown_signal_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        default:
            return -1;
...
int gb_unknown_signal_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        default:
            return -1;
...
int gb_unknown_signal_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
