...
#include "nbe_pu_gbf.h"
#include "nbp_application.h"

#define NPCTX      struct application_ctx*
...
void *application_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct application_ctx));
    if (!pState) {
...
int application_get_param(void *pState, int index, void *param, int len) {
    ...
    switch (index) {
        case 1:
           memcpy(param, &pctx->frame_length, len);
           break;
        default:
            return -1;
    }
...
int application_set_param(void *pState, int index, void *param, int len) {
    ...
    switch (index) {
        case 1:
           memcpy(&pctx->frame_length, param, len);
           break;
        default:
            return -1;
    }
