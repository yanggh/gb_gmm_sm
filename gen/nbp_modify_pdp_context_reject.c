...
#include "nbe_pu_gbf.h"
#include "nbp_modify_pdp_context_reject.h"

#define NPCTX      struct modify_pdp_context_reject_ctx*
...
struct modify_pdp_context_reject_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "SM", "MODIFY PDP CONTEXT REJECT");
...
void *modify_pdp_context_reject_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct modify_pdp_context_reject_ctx));
    if (!pState) {
