...
#include "nbe_pu_gbf.h"
#include "nbp_request_secondary_pdp_context_activation_reject.h"

#define NPCTX      struct request_secondary_pdp_context_activation_reject_ctx*
...
struct request_secondary_pdp_context_activation_reject_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "SM", "REQUEST SECONDARY PDP CONTEXT ACTIVATION REJECT");
...
void *request_secondary_pdp_context_activation_reject_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct request_secondary_pdp_context_activation_reject_ctx));
    if (!pState) {
