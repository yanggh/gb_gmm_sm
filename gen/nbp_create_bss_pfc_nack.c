...
#include "nbe_pu_gbf.h"
#include "nbp_create_bss_pfc_nack.h"

#define NPCTX      struct create_bss_pfc_nack_ctx*
...
struct create_bss_pfc_nack_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "CREATE-BSS-PFC-NACK");
...
void *create_bss_pfc_nack_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct create_bss_pfc_nack_ctx));
    if (!pState) {
