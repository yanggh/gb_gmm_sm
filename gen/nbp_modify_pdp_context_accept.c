...
#include "nbe_pu_gbf.h"
#include "nbp_modify_pdp_context_accept.h"

#define NPCTX      struct modify_pdp_context_accept_ctx*
...
struct modify_pdp_context_accept_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
extern "C" {
#endif
void *modify_pdp_context_accept_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct modify_pdp_context_accept_ctx));
    if (!pState) {
...
void modify_pdp_context_accept_state_free(void *state) {
...
nbe_pu_gb *modify_pdp_context_accept_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int modify_pdp_context_accept_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int modify_pdp_context_accept_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int modify_pdp_context_accept_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
