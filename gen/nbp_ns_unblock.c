...
#include "nbe_pu_gbf.h"
#include "nbp_ns_unblock.h"

#define NPCTX      struct ns_unblock_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("1", "NS Layer", "NS-UNBLOCK");
...
void *ns_unblock_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct ns_unblock_ctx));
    if (!pState) {
