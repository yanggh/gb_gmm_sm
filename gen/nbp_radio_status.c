...
#include "nbe_pu_gbf.h"
#include "nbp_radio_status.h"

#define NPCTX      struct radio_status_ctx*
...
extern "C" {
#endif
void *radio_status_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct radio_status_ctx));
    if (!pState) {
...
void radio_status_state_free(void *state) {
...
nbe_pu_gb *radio_status_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int radio_status_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int radio_status_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int radio_status_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
