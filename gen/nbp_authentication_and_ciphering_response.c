...
#include "nbe_pu_gbf.h"
#include "nbp_authentication_and_ciphering_response.h"

#define NPCTX      struct authentication_and_ciphering_response_ctx*
...
struct authentication_and_ciphering_response_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
extern "C" {
#endif
void *authentication_and_ciphering_response_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct authentication_and_ciphering_response_ctx));
    if (!pState) {
...
void authentication_and_ciphering_response_state_free(void *state) {
...
nbe_pu_gb *authentication_and_ciphering_response_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int authentication_and_ciphering_response_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int authentication_and_ciphering_response_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int authentication_and_ciphering_response_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
