...
#include "nbe_pu_gbf.h"
#include "nbp_suspend_nack.h"

#define NPCTX      struct suspend_nack_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "SUSPEND-NACK");
...
void *suspend_nack_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct suspend_nack_ctx));
    if (!pState) {
