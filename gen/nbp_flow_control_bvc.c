...
#include "nbe_pu_gbf.h"
#include "nbp_flow_control_bvc.h"

#define NPCTX      struct flow_control_bvc_ctx*
...
extern "C" {
#endif
void *flow_control_bvc_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct flow_control_bvc_ctx));
    if (!pState) {
...
void flow_control_bvc_state_free(void *state) {
...
nbe_pu_gb *flow_control_bvc_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int flow_control_bvc_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int flow_control_bvc_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int flow_control_bvc_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
