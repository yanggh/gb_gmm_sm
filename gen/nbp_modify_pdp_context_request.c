...
#include "nbe_pu_gbf.h"
#include "nbp_modify_pdp_context_request.h"

#define NPCTX      struct modify_pdp_context_request_ctx*
...
struct modify_pdp_context_request_ctx {
    scan_state __ss;
    nbe_pu_gb *gb;
...
{{ 
	...
	BEGIN_SIGNAL("4", "SM", "MODIFY PDP CONTEXT REQUEST");
...
void *modify_pdp_context_request_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct modify_pdp_context_request_ctx));
    if (!pState) {
