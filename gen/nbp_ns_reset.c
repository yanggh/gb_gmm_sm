...
#include "nbe_pu_gbf.h"
#include "nbp_ns_reset.h"

#define NPCTX      struct ns_reset_ctx*
...
extern "C" {
#endif
void *ns_reset_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct ns_reset_ctx));
    if (!pState) {
...
void ns_reset_state_free(void *state) {
...
nbe_pu_gb *ns_reset_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int ns_reset_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int ns_reset_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int ns_reset_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
