...
#include "nbe_pu_gbf.h"
#include "nbp_download_bss_pfc.h"

#define NPCTX      struct download_bss_pfc_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("2", "BSSGP Layer", "DOWNLOAD-BSS-PFC");
...
void *download_bss_pfc_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct download_bss_pfc_ctx));
    if (!pState) {
