...
#include "nbe_pu_gbf.h"
#include "nbp_ns_alive_ack.h"

#define NPCTX      struct ns_alive_ack_ctx*
...
{{ 
      ...
      BEGIN_SIGNAL("1", "NS Layer", "NS-ALIVE-ACK");
...
extern "C" {
#endif
void *ns_alive_ack_state_new(nbp_pd_extract_context *extctx){
    NPCTX pState = (NPCTX) malloc(sizeof(struct ns_alive_ack_ctx));
    if (!pState) {
...
void ns_alive_ack_state_free(void *state) {
...
nbe_pu_gb *ns_alive_ack_get_userdata(void *pState) {
    return ((NPCTX)pState)->gb;
}
int ns_alive_ack_get_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int ns_alive_ack_set_param(void *pState, int index, void *param, int len) {
    NPCTX pctx = (NPCTX) pState;
    switch (index) {
        case 1:
...
int ns_alive_ack_parse(void *pState, uint8_t *input, uint32_t input_len, int flag, void *ctx) {
    NPCTX pctx = (NPCTX) pState;
    uint32_t plen = 0;
    int match = -1;
    if (pctx->__g == 0) {
