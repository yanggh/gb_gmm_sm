...
void fr_layer(uint32_t len)
{
    ...
    if(flag == 1)   //Default address field format (2 octets)
    {
        ...
        if (dlci_value >= 16 && dlci_value <= 1007)
	    {
            ...
            SET_STR(gbe, element_name, "user_data");
            SET_STR(gbe, element_desc, "USER DATA");
	    	POS(pos);
	...
	if (dlci_value == 0)
	{
        ...
        if (tmp == 125)
        {
            ...
            SET_STR(gbe, element_name, "status");
            SET_STR(gbe, element_desc, "STATUS");
			status();
			//control_field();
        }
        else if(tmp == 117)
        {
            ...
            SET_STR(gbe, element_name, "status enquiry");
            SET_STR(gbe, element_desc, "STATUS ENQUIRY");
			status_enquiry();
			//control_field();
        }
        else
...
int address_field(uint32_t len)
{
...
void control_field()
{
	...
	if (tmp == 0 || tmp == 2)  //I format
	{
		SET_STR(fr_msg, frame_type, "I format");
		A_B8_BIT1(gbf, ns, "Transmitter send sequence number", 1, 7);
		A_B8_BIT1(gbf, spare, "Spare", 8, 8);
		SKIP(1);
	...
	{
		int tmp1;
		SET_STR(fr_msg, frame_type, "U format");
		B8_BIT(tmp1, 5, 5);
		A_B8_BIT1(gbf, m1, "Modifier function bit", 6, 8);
		A_B8_BIT3(gbf, p_f, "Final bit", 5, 5, p_dict);
		A_B8_BIT1(gbf, m2, "Modifier function bit", 3, 4);
	...
	{
		SET_STR(fr_msg, frame_type, "S format");
		A_B8_BIT1(gbf, reserved, "Reserved", 5, 8);
		A_B8_BIT1(gbf, su, "Supervisory function bit", 3, 4);
		A_B8_BIT1(gbf, spare, "Spare", 1, 2);
		SKIP(1);
...
void pd()
{
	BEGIN_ELEMENT("Protocol discriminator", pd); 
...
void call_reference()
{
	BEGIN_ELEMENT("Call reference", call_reference); 
...
int message_type()
{
	BEGIN_ELEMENT("Message type", message_type); 
...
int report_type()
{
	BEGIN_ELEMENT("Report type", report_type); 
...
void pvc_status()
{
	BEGIN_ELEMENT("PVC status", pvc_status); 
...
void link_intefrity_verification()
{
	BEGIN_ELEMENT("Link integrity verification", link_intefrity_verification); 
